 ...
 */
function verificarPin(organizationId, usuario, pin) {
      ...
      if (row.pin.startsWith('$2a$') || row.pin.startsWith('$2b$')) {
        ...
        if (match) {
          bcrypt.hash(pin, BCRYPT_ROUNDS, (hashErr, hash) => {
            if (!hashErr) db.run('UPDATE pins SET pin = ? WHERE organizationId = ? AND usuario = ?', [hash, organizationId, usuario]);
          });
        }
        resolve(match);
      }
...
router.post('/solicitacoes-retirada', (req, res) => {
  ...
  db.run(
    `INSERT INTO solicitacoes_retirada (
      id, tipo, registroIds, loja, valorTotal, responsavel, dataRetirada, solicitadoPor, status, criadoEm, organizationId
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pendente', ?, ?)`,
    [id, tipo, JSON.stringify(registroIds), loja || null, Number(valorTotal) || 0, responsavel, dataRetirada, actorUsuario, criadoEm, organizationId],
    function (err) {
      ...
      publish('retirada.solicitada', payload, { usuario: actorUsuario, organizationId });
...
router.get('/solicitacoes-retirada', (req, res) => {
  ...
  const organizationId = organizationIdDe(req);
  const sql = status
    ? 'SELECT * FROM solicitacoes_retirada WHERE status = ? AND organizationId = ? ORDER BY criadoEm DESC'
    : 'SELECT * FROM solicitacoes_retirada WHERE organizationId = ? ORDER BY criadoEm DESC LIMIT 100';
  const params = status ? [status, organizationId] : [organizationId];
...
router.post('/solicitacoes-retirada/:id/autorizar', requireOwner, async (req, res) => {
  ...
  try {
    const pinValido = await verificarPin(organizationId, actorUsuario, pin);
    if (!pinValido) {
  ...
  db.get('SELECT * FROM solicitacoes_retirada WHERE id = ? AND organizationId = ?', [id, organizationId], (err, row) => {
    if (err) return res.status(500).json({ error: err.message });
    ...
    db.run(
      `UPDATE ${tabela} SET status = ?, dataRetirada = ?, retiradoPor = ?, confirmadoPorApp = ?, autorizadoPor = ? WHERE id IN (${placeholdersIds}) AND organizationId = ?`,
      [camposRetirada.status, camposRetirada.dataRetirada, camposRetirada.retiradoPor, camposRetirada.confirmadoPorApp, camposRetirada.autorizadoPor, ...solicitacao.registroIds, organizationId],
      (errUpdate) => {
        if (errUpdate) {
        ...
        solicitacao.registroIds.forEach(registroId => {
          publish(eventoRegistroAlterado(solicitacao.tipo), { id: registroId, campos: camposRetirada }, { usuario: actorUsuario, organizationId });
        });

        db.run(
          `UPDATE solicitacoes_retirada SET status = 'aprovada', autorizadoPor = ?, respondidoEm = ? WHERE id = ? AND organizationId = ?`,
          [actorUsuario, respondidoEm, id, organizationId],
          (errFinal) => {
            ...
            publish('retirada.autorizada', { id, tipo: solicitacao.tipo, registroIds: solicitacao.registroIds, autorizadoPor: actorUsuario, solicitadoPor: solicitacao.solicitadoPor }, { usuario: actorUsuario, organizationId });
...
router.post('/solicitacoes-retirada/:id/recusar', requireOwner, (req, res) => {
  ...
  db.get('SELECT * FROM solicitacoes_retirada WHERE id = ? AND organizationId = ?', [id, organizationId], (err, row) => {
    ...
    const respondidoEm = new Date().toISOString();
    db.run(
      `UPDATE solicitacoes_retirada SET status = 'recusada', autorizadoPor = ?, motivoRecusa = ?, respondidoEm = ? WHERE id = ? AND organizationId = ?`,
      [actorUsuario, motivo || null, respondidoEm, id, organizationId],
      (errUpdate) => {
        ...
        publish('retirada.recusada', { id, tipo: solicitacao.tipo, registroIds: solicitacao.registroIds, autorizadoPor: actorUsuario, solicitadoPor: solicitacao.solicitadoPor, motivo: motivo || null }, { usuario: actorUsuario, organizationId });
