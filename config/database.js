...
function initDb(onSuccess) {
  ...
  db.all(checkSql, [], (err, rows) => {
    ...
    const startInitialization = () => {
      ...
      // colaboradores.nome é único globalmente (ON CONFLICT(nome) em
      // routes/auth.js depende deste índice) — garante em instalações onde a
      // tabela foi criada sem a constraint UNIQUE original.
      promise = promise.then(() => {
        return new Promise(resolve => {
          db.run('CREATE UNIQUE INDEX IF NOT EXISTS idx_colaboradores_nome ON colaboradores(nome)', [], (err) => {
            if (err) console.error('Erro ao criar índice único colaboradores.nome:', err.message);
            resolve();
          });
        });
      });
      ...
      // Campo editável de e-mail do colaborador, substituindo o EMAIL_MAP
      // hardcoded de routes/auth.js (3 Gmails pessoais direto no código).
      promise = promise.then(() => {
        return new Promise(resolve => {
