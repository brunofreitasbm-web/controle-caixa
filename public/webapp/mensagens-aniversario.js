...
//
// Em vez de 1500 textos soltos (impossíveis de manter), a mensagem é montada
// por 4 partes combinadas:
//   10 aberturas × 5 votos × 10 convites × 3 fechamentos = 1.500 mensagens.
// Cada clique em "Enviar Parabéns" sorteia uma combinação diferente.
...
// Monta as combinações fixas de cada bloco (com os placeholders ainda
// presentes — as variáveis só são preenchidas na hora do envio).
const BLOCO_A = A_ABERTURAS.flatMap(abertura =>
  A_VOTOS.map(voto => `${abertura} ${voto}`)
);

const BLOCO_B = B_CONVITES.flatMap(convite =>
  B_FECHAMENTOS.map(fechamento => `${convite} ${fechamento}`)
);

const TOTAL_MENSAGENS_ANIVERSARIO = BLOCO_A.length * BLOCO_B.length; // 1500
